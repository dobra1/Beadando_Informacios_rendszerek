...
public class BlueMessageConsumer {
            ...
            consumer.setMessageListener(message -> {
                try {
                    if (message instanceof TextMessage) {
                        ...
                        System.out.println("KÉK üzenet fogadva: " + text);

                        if (new Random().nextInt(10) < 3) {
                            System.out.println("Hibát szimulálunk (rollback)");
                            session.rollback();
                            return;
                        }
    ...
    private static void sendStatistics(Destination statisticsQueue, Session session) {
        try {
            ...
            TextMessage statsMsg = session.createTextMessage("10 KÉK üzenet feldolgozva");
            producer.send(statsMsg);
            System.out.println("Statisztika elküldve: 10 KÉK üzenet feldolgozva.");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
